...
export async function ensureSchema(env) {
  ...
  const hasR2Key = await columnExists(
    env,
    "text_images",
    "r2_key"
  );
...
export async function getPublicTextById(
  env,
  id
) {
  return await env.DB
...
export async function getNewCommentCount(
  env
) {
  return getNewNotificationCount(env);
}
